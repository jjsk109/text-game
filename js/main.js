 ...
 *  Sub-episode (덮어쓰기 병합)
 *  ========================= */
const SUB_EPISODE_BASE = "/episode/subEpisode"; // 주신 경로 그대로 (철자 유지)

async function loadEpisodeScenes(episodeName) {
    const url = `${SUB_EPISODE_BASE}/${encodeURIComponent(episodeName)}.json`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) throw new Error(`서브 에피소드 로드 실패 (${res.status}) : ${url}`);
    const data = await res.json();
    if (!Array.isArray(data)) throw new Error("서브 에피소드 형식 오류: 배열이어야 합니다");
    return data;
}

/**
 * 기존 story에 서브 에피소드 씬들을 덮어쓰기 병합한다.
 * - 동일 scene_id는 완전 교체(덮어쓰기)
 * - 없는 scene_id는 뒤에 추가
 */
function mergeEpisodeScenesIntoState(scenes) {
    console.log("mergeEpisodeScenesIntoState ", scenes);

    if (!Array.isArray(scenes) || scenes.length === 0) return;

    // 1) sceneIndex에 먼저 반영(덮어쓰기)
    for (const s of scenes) {
        if (!s || !s.scene_id) continue;
        GameState.sceneIndex.set(s.scene_id, s);
    }

    // 2) GameState.story에도 반영(덮어쓰기 or 추가)
    const idxById = new Map();
    GameState.story.forEach((s, i) => idxById.set(s.scene_id, i));

    for (const s of scenes) {
        console.log("stroy : ", s);

        if (!s || !s.scene_id) continue;
        if (idxById.has(s.scene_id)) {
            // 덮어쓰기
            const index = idxById.get(s.scene_id);
            GameState.story[index] = s;
        } else {
            // 신규 추가
            idxById.set(s.scene_id, GameState.story.length);
            GameState.story.push(s);
        }
    }
}
 ...
 * - 호출한 쪽에서 await로 호출
 */
async function applySubEpisodeIfAny(episodeName) {
    console.log("episodeName : ", episodeName);

    if (!episodeName) return;
    const scenes = await loadEpisodeScenes(episodeName);
    mergeEpisodeScenesIntoState(scenes);
}
...
async function loadStory() {
    try {
        const res = await fetch("/episode/mainEpisode/story_flow.json", { cache: "no-store" });
...
function getScene(sceneId) {
    console.log("getScene GameState : ", GameState);
...
function goToScene(sceneId) {
    ...
    console.log("goToScene nextScene : ", nextScene);
...
function renderChoices(scene) {
    ...
    scene.choice.forEach((ch) => {
        ...
        btn.addEventListener("click", async () => {
            ...
            if (episodeKey) {
                try {
                    await applySubEpisodeIfAny(episodeKey);
                } catch (e) {
                    console.error(e);
                    // 실패 시에도 게임은 계속 진행 가능하게만 처리
                    safeSetText(dom.sceneDesc, `서브 에피소드 로드 실패: ${e.message}`);
                }
            }
